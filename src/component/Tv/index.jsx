...
import "../Movies/movies.css"; // shared styles with the Movies page
...
const TABS = [
  { id: "trending", label: "Trending", endpoint: "/trending/tv/day" },
...
export default function Tv() {
  // Filters live in the URL, so Back from a show page restores them
  ...
  useEffect(() => {
    ...
    if (isFiltering) {
      ...
      if (serverSort === "rating") query["vote_count.gte"] = 100; // skip 10/10 shows with 2 votes
    } else {
  ...
  return (
    <div className="discover container">
      <header className="discover-head">
        <h1 className="discover-title">Discover TV shows</h1>
        <p className="discover-sub">
          Browse what's trending, top rated and airing now, or narrow it down by
          genre, language and first air year.
        </p>
      </header>
      ...
      <div className="toolbar">
        {isFiltering ? (
          <p className="result-count" aria-live="polite">
            {status === "ready"
              ? `${data.totalResults.toLocaleString()} shows match your filters`
              : "Finding shows…"}
          </p>
        ) : (
          <div className="segmented" role="tablist" aria-label="TV lists">
            {TABS.map((t) => (
              <button
                key={t.id}
                role="tab"
        ...
        <div className="toolbar-controls">
          <Select label="First air year" value={year} onChange={(v) => update({ year: v })}>
      ...
      {/* Results */}
      {status === "loading" && (
        <div className="row g-4" aria-busy="true" aria-label="Loading TV shows">
      ...
      {status === "error" && (
        <div className="state">
          ...
          <h2 className="state-title">TV shows didn't load</h2>
      ...
      {status === "ready" && shows.length === 0 && (
        <div className="state">
          <i className="fa-solid fa-tv state-icon" aria-hidden="true"></i>
          <h2 className="state-title">No shows match these filters</h2>
